...
class CRUDBase(Generic[ModelType, CreateSchemaType, UpdateSchemaType]):
    ...
    def get_by_field(
        ...
        if column is None:
            raise ValueError(
                f"Field {field} not found in model {self.model.__tablename__}"
            )
    ...
    def get_all(self, session: Session, *, order_by: Any = None) -> list[ModelType]:
        statement = select(self.model)
        if order_by is not None:
            statement = statement.order_by(order_by)
        return list(session.exec(statement).all())
    ...
    def exists(self, session: Session, id: Any) -> bool:
        statement = (
            select(func.count()).select_from(self.model).where(col(self.model.id) == id)
        )
    ...
    def upsert(
        ...
        *,
        ...
        if update_fields:
            ...
            update_dict = {k: insert_stmt.excluded[k] for k in data.keys() if k != "id"}
