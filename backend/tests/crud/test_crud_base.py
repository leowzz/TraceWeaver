...
@pytest.fixture(scope="function")
def sample_hero(session, hero_crud):
...
# Tests
class TestCreate:
    def test_create_with_schema(self, session, hero_crud):
    ...
    def test_create_with_dict(self, session, hero_crud):
    ...
    def test_create_many(self, session, hero_crud):
    ...
    def test_create_many_empty_list(self, session, hero_crud):
        """Test create_many with empty list."""
        count = hero_crud.create_many(session, [])
        assert count == 0


class TestRead:
    def test_get_existing(self, session, hero_crud, sample_hero):
    ...
    def test_get_non_existing(self, session, hero_crud):
        """Test getting a non-existing record."""
        hero = hero_crud.get(session, 99999)
        assert hero is None

    def test_get_by_ids(self, session, hero_crud):
    ...
    def test_get_by_field(self, session, hero_crud, sample_hero):
        """Test getting a record by specific field."""
        hero = hero_crud.get_by_field(session, "name", "Deadpond")
        assert hero is not None
        assert hero.id == sample_hero.id

    def test_get_by_field_non_existing(self, session, hero_crud):
        """Test getting by field with non-existing value."""
        hero = hero_crud.get_by_field(session, "name", "NonExistent")
        assert hero is None

    def test_get_by_field_invalid_field(self, session, hero_crud):
        """Test getting by invalid field raises error."""
        with pytest.raises(ValueError, match="Field invalid_field not found"):
            hero_crud.get_by_field(session, "invalid_field", "value")
    ...
    def test_get_multi_with_order(self, session, hero_crud):
    ...
    def test_get_all_with_order(self, session, hero_crud):
...
class TestPagination:
    ...
    def test_has_more_page_with_last_id(self, session, hero_crud):
...
class TestUpdate:
    def test_update_with_schema(self, session, hero_crud, sample_hero):
    ...
    def test_update_non_existing(self, session, hero_crud):
        """Test updating a non-existing record."""
        result = hero_crud.update(session, 99999, HeroUpdate(name="Test"))
        assert result is None

    def test_update_exclude_unset(self, session, hero_crud, sample_hero):
...
class TestDelete:
    ...
    def test_delete_non_existing(self, session, hero_crud):
        """Test deleting a non-existing record."""
        deleted = hero_crud.delete(session, 99999)
        assert deleted is None

    def test_delete_many(self, session, hero_crud):
        ...
        assert hero_crud.get(session, hero3.id) is None

    def test_delete_many_empty_list(self, session, hero_crud):
        """Test delete_many with empty list."""
        count = hero_crud.delete_many(session, [])
        assert count == 0
...
class TestUtility:
    ...
    def test_exists_true(self, session, hero_crud, sample_hero):
        ...
        assert hero_crud.exists(session, sample_hero.id) is True

    def test_exists_false(self, session, hero_crud):
        """Test exists returns False for non-existing record."""
        assert hero_crud.exists(session, 99999) is False
